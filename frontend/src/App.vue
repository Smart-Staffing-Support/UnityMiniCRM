...
const fetchNotifications = async () => {
  try {
    const res = await crmService.getNotifications();
    notifications.value = Array.isArray(res)
      ? res.map((n) => ({
        ...n,
        timeline_event: n.timeline_event || { description: "No description" },
      }))
      : [];
  } catch (error) {
...
const deleteNotification = async (notificationId) => {
  try {
    await crmService.deleteNotification(notificationId);
...
<template>
  <v-app>
    <template v-if="showLayout && isAuthenticated">
      <!-- Premium App Bar -->
      <v-app-bar elevation="0" class="app-bar glass-effect" height="80">
        <div class="d-flex align-center w-100 px-4">
          <!-- Logo & Brand -->
          <div class="d-flex align-center">
            <v-btn icon @click="drawer = !drawer" class="mr-3" variant="text" size="large">
          ...
          <div class="search-wrapper mx-4 d-none d-sm-block">
            <v-text-field prepend-inner-icon="mdi-magnify" placeholder="Search contacts, deals, companies..."
              variant="solo" density="comfortable" hide-details flat class="search-field" bg-color="grey-lighten-4">
          ...
          <div class="d-flex align-center gap-2">
            ...
            <v-menu offset-y>
              <template v-slot:activator="{ props }">
                <v-btn icon v-bind="props" class="action-btn" variant="tonal" color="primary">
                  <v-icon>mdi-plus-circle</v-icon>
                </v-btn>
              </template>
              <v-list min-width="200">
                <v-list-subheader>Quick Add</v-list-subheader>
                <v-list-item prepend-icon="mdi-account-plus" title="New Contact"
                  @click="router.push('/contacts')"></v-list-item>
                <v-list-item prepend-icon="mdi-office-building-plus" title="New Company"
                  @click="router.push('/companies')"></v-list-item>
            ...
            <v-menu offset-y :close-on-content-click="false">
              <template v-slot:activator="{ props }">
                <v-btn icon v-bind="props" class="action-btn" variant="text">
                  <v-badge color="error" :content="unreadCount" offset-x="12" offset-y="12" v-if="unreadCount > 0">
              ...
              <v-list style="min-width: 300px; max-height: 400px; overflow-y: auto;">
                ...
                <template v-for="notif in notifications" :key="notif.id">
                  <v-list-item @click="toggleExpand(notif.id)" :class="{ 'bg-grey-lighten-3': !notif.read }">
                    <v-list-item-content>
                      <v-list-item-title>
                        {{ notif.timeline_event?.description || "No description" }}
                      </v-list-item-title>
                    ...
                    <!-- Chevron indicating expanded/collapsed -->
                    <v-list-item-icon>
                      <v-icon :class="{ 'rotate-180': expanded === notif.id }">mdi-chevron-down</v-icon>
                    </v-list-item-icon>

                    <!-- Delete button -->
                    <v-list-item-icon>
                      <v-btn icon small @click.stop="deleteNotification(notif.id)">
                        <v-icon color="red" size="18">mdi-delete</v-icon>
                      </v-btn>
                    </v-list-item-icon>
                  </v-list-item>
                  ...
                  <!-- Expanded content -->
                  <v-expand-transition>
                    <div v-if="expanded === notif.id" class="px-4 py-3 text-body-2 text-grey-darken-2"
                      style="background: #fafafa; border-left: 3px solid #1976d2;">
                      <div><strong>Details:</strong></div>
                      <div class="mt-1">
                        {{ notif.timeline_event?.extra_details || "No additional info" }}
                      </div>
                    </div>
                  </v-expand-transition>
              ...
              <v-card class="user-menu-card">
                <v-card-text class="pa-4">
                  ...
                  <v-list density="compact" class="bg-transparent">
                    <v-list-item prepend-icon="mdi-account-circle" title="My Profile"
                      subtitle="View your details"></v-list-item>
                    <v-list-item prepend-icon="mdi-cog" title="Settings" subtitle="Preferences"></v-list-item>
                    <v-list-item prepend-icon="mdi-help-circle" title="Help & Support"
                      subtitle="Get assistance"></v-list-item>
      ...
      <v-navigation-drawer v-model="drawer" :rail="rail" permanent class="navigation-drawer" width="280">
        ...
        <div class="drawer-header pa-4">
          ...
          <v-btn v-else icon size="small" variant="text" color="white" @click="rail = false" class="mx-auto">
            <v-icon>mdi-menu</v-icon>
          </v-btn>

          <v-btn v-if="!rail" icon size="small" variant="text" color="white" @click="rail = true" class="collapse-btn">
        ...
        <v-list class="nav-list pa-3" nav>
          <v-list-item v-for="item in menuItems" :key="item.route" :to="item.route" :active="route.path === item.route"
            class="nav-item mb-2" rounded="xl" :style="{ background: getItemGradient(item) }">
            <template v-slot:prepend>
              <div class="nav-icon-wrapper" :style="{
                background:
                  route.path === item.route
                    ? 'rgba(255,255,255,0.2)'
                    : 'rgba(255,255,255,0.15)',
              }">
                <v-icon :color="route.path === item.route ? 'white' : 'white'" size="22">
                  {{ item.icon }}
                </v-icon>
              </div>
            </template>
            ...
            <v-list-item-subtitle v-if="!rail" :class="route.path === item.route
          ...
          <div class="pa-4">
            ...
            <v-card variant="outlined" class="help-card" rounded="lg">
              <v-card-text class="pa-4 text-center">
                ...
                <div class="text-caption text-white opacity-80 mb-3">
