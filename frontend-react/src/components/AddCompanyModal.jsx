...
export default function AddCompanyModal({ isOpen, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    setForm({ name: "", industry: "", email: "", phone: "", website: "", address: "" });
    setError("");
  }, [isOpen]);
  ...
  const handleSave = async (e) => {
    ...
    setError("");
    try {
      await crmService.createCompany(form);
      onSaved();
      onClose();
    } catch (err) {
      const data = err.response?.data;
      setError(typeof data === "string" ? data : JSON.stringify(data) || "Failed to save company");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
        <div className="flex items-center gap-2 px-6 py-4 bg-gray-50 rounded-t-xl border-b border-gray-200">
          ...
          <span className="text-lg font-bold text-gray-800">New Company</span>
        </div>

        <form onSubmit={handleSave} className="p-6">
          <div className="mb-4">
            <label className={label}>Company Name *</label>
            <input required className={field} value={form.name} onChange={set("name")} placeholder="Acme Corp" />
          </div>

          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className={label}>Industry</label>
              <input className={field} value={form.industry} onChange={set("industry")} placeholder="Technology" />
            </div>
            <div>
              <label className={label}>Email</label>
              <input type="email" className={field} value={form.email} onChange={set("email")} placeholder="info@acme.com" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input className={field} value={form.website} onChange={set("website")} placeholder="https://acme.com" />
            </div>
          </div>
